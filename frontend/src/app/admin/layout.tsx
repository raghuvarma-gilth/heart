...
import { FiHeart, FiHome, FiUsers, FiCalendar, FiBarChart2, FiLogOut, FiSun, FiMoon, FiMenu, FiX } from 'react-icons/fi';

const navItems = [
  { href: '/admin', icon: FiHome, label: 'Overview' },
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (!loading && userData && userData.role === 'patient') router.push('/dashboard');
  ...
  if (loading) return (
    <div className="min-h-screen gradient-hero flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-white/40 text-sm mt-4">Loading Admin Panel...</p>
      </div>
    </div>
  );
    ...
    <div className={`min-h-screen flex ${dark ? 'gradient-hero text-white' : 'bg-slate-50 text-gray-900'}`}>
      ...
      <aside className={`fixed left-0 top-0 bottom-0 w-64 z-40 flex flex-col transition-transform duration-200 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 ${dark ? 'glass border-r border-white/10' : 'bg-white border-r border-gray-200 shadow-lg'}`}>
        <div className="p-5 flex items-center gap-3">
          ...
          <div>
            <span className="font-bold text-lg tracking-tight block leading-tight">CardioMind</span>
        ...
        <nav className="flex-1 px-3 py-4 space-y-1">
          {navItems.map(item => {
            ...
            return (
              <Link key={item.href} href={item.href} className={`flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-medium transition-colors duration-150 ${
                active
                  ? dark
        ...
        <div className={`p-4 border-t ${dark ? 'border-white/10' : 'border-gray-200'}`}>
          <div className="flex items-center gap-3 mb-3 px-2">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-red-500 to-rose-600 flex items-center justify-center text-xs font-bold text-white">
              {userData?.name?.charAt(0) || 'A'}
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium truncate">{userData?.name}</p>
              <p className={`text-xs truncate ${dark ? 'text-white/40' : 'text-gray-400'}`}>Administrator</p>
            </div>
          </div>
          <div className="flex gap-2">
            <button onClick={toggle} className={`flex-1 py-2 rounded-lg text-xs font-medium transition-colors duration-150 ${dark ? 'bg-white/5 hover:bg-white/10 text-white/60' : 'bg-gray-100 hover:bg-gray-200 text-gray-600'}`}>
              {dark ? <FiSun className="inline mr-1" /> : <FiMoon className="inline mr-1" />}{dark ? 'Light' : 'Dark'}
            </button>
